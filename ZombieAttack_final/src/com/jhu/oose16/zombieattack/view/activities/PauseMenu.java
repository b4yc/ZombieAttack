...
public class PauseMenu extends Menu<PauseMenu.ViewComponentTypes> {
	...
	PMCommand pmReturn = new PMReturn();
	PMCommand pmRetry = new PMRetry();
	PMCommand pmReturnMainMenu = new PMReturnMainMenu();
	PMCommand pmActionUp = new PMActionUp();
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (viewComponentManager
			...
			pmReturn.execute(viewComponentManager, event, PauseMenu, GamePlay);
		...
		if (viewComponentManager
				...
				.getViewRect().isInside(touchPoint)) {
			pmRetry.execute(viewComponentManager, event, PauseMenu, GamePlay);
			// if (event.getAction() == MotionEvent.ACTION_DOWN) {
			// 	((Button) viewComponentManager
			// 			.getViewComponent(ViewComponentTypes.RETRY_BUTTON))
			// 			.buttonDown();
		...
		if (viewComponentManager
				...
				.getViewRect().isInside(touchPoint)) {
			pmReturnMainMenu.execute(viewComponentManager, event, PauseMenu, GamePlay);
			// if (event.getAction() == MotionEvent.ACTION_DOWN) {
			// 	((Button) viewComponentManager
			// 			.getViewComponent(ViewComponentTypes.RETURN_MAIN_MENU_BUTTON))
			// 			.buttonDown();
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			pmActionUp.execute(viewComponentManager);
			// ((Button) viewComponentManager
			// 		.getViewComponent(ViewComponentTypes.RETURN_BUTTON))
			// 		.buttonUp();
